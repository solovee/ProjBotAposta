...
def agendar_atualizacao_csv():
    ...
    # Se estiver fora do horário 08:00–17:00 → atualiza e agenda a cada 30 min
    if hora_atual < inicio_dia or hora_atual > fim_dia:
        atualizar_csv_dia_atual()
        intervalo = 60 * 30  # 30 minutos
        logger.info("⏳ Próxima atualização em 30 minutos")
        threading.Timer(intervalo, agendar_atualizacao_csv).start()
    else:
        # Dentro do horário 08:00–17:00 → pausa até as 17:00
        proxima_execucao = datetime.combine(agora.date(), fim_dia, tz)
        intervalo = (proxima_execucao - agora).total_seconds()
        logger.info(f"⏳ Dentro do horário comercial. Próxima execução às {proxima_execucao}")
        threading.Timer(intervalo, agendar_atualizacao_csv).start()
